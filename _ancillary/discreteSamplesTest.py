...
p_lowC  = np.zeros((ITERS,len(events)))
p_uppC  = np.zeros((ITERS,len(events)))
...
for it in range(ITERS):
    ...
    for idx in range(len(events)):
        ...
        beta_bar = beta/np.ceil(throws/2)
        
        if k == throws:
            epsilon_low = computeBetaPPF(N=throws, k=k-1, d=1, beta=beta_bar)
            epsilon_upp = 1
            
        else:
            epsilon_upp = computeBetaPPF(throws, k, d, 1-beta_bar)
        
            if k != 0:
                epsilon_low = computeBetaPPF(throws, k-1, d, beta_bar)
            else:
                epsilon_low = 0
    ...
    # print('\nold LB:',p_low[it])
    # print('old UB:',p_upp[it])
    
    # print('new LB:',p_lowB[it])
    # print('new UB:',p_uppB[it])
    ...
    c = np.sqrt(np.log(beta/2)/(-2*throws))
    p_lowC[it] = np.maximum(0, scores[it]/throws - c)
    p_uppC[it] = np.minimum(1, scores[it]/throws + c)
...
p_low_meanB  = np.mean(p_lowB, axis=0)
p_low_stdevB = np.std(p_lowB, axis=0)
p_upp_meanB  = np.mean(p_uppB, axis=0)
p_upp_stdevB = np.std(p_uppB, axis=0)

p_low_meanC  = np.mean(p_lowC, axis=0)
p_low_stdevC = np.std(p_lowC, axis=0)
p_upp_meanC  = np.mean(p_uppC, axis=0)
p_upp_stdevC = np.std(p_uppC, axis=0)
...
# Plot Hoeffding bounds
sns.pointplot(data=pd.DataFrame(p_lowC), dodge=True, join=False, ci='sd',
              color='green')

sns.pointplot(data=pd.DataFrame(p_uppC), dodge=True, join=False, ci='sd',
              color='green')
