...
# -*- coding: utf-8 -*-
"""
Created on Thu May 27 14:01:54 2021

@author: tbadings
"""

class UAV_OLD(master.LTI_master):
...
class building_2room_OLD(master.LTI_master):
    ...
    def __init__(self):
        ...
        # Partition size
        self.setup['partition']['nrPerDim']  = [10,10,11,11] #29,29]
        self.setup['partition']['width']     = [0.4, 0.4, 0.05, 0.05]
        self.setup['partition']['origin']    = [21, 21, 42.515, 62.567] #42.3019313 , 62.86740522] #[22.93,  18.53,  37.29, 37.045]
    ...
    def setModel(self, observer):           
        ...
        W_cont  = np.array([
                [ 0.8 + (Twss/(BAS.Zone1['Rn']*BAS.Zone1['Cz'])) + (alpha1_z1)/(BAS.Zone1['Cz']) ],
                [ 0.05 + ((Twss-1)/(BAS.Zone2['Rn']*BAS.Zone2['Cz'])) + (alpha1_z2)/(BAS.Zone1['Cz']) ],
                [ 0 ],
                [ 0 ]
                ])
        ...
        self.noise = dict()
        self.noise['w_cov'] = 1.0*np.diag([ BAS.Zone1['Tz']['sigma'], BAS.Zone2['Tz']['sigma'], 0.0001*BAS.Radiator['rw']['sigma'], 0.0001*BAS.Radiator['rw']['sigma'] ])
